...
export const uploadAPI = {
  ...
  uploadGalleryImage: (file) => {
    const formData = new FormData()
    formData.append('image', file)
    return api.post('/admin/upload/gallery', formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
      },
    })
  },
  deleteGalleryImage: (filename) => api.delete(`/admin/upload/gallery/${filename}`),
}
...
// Quick Tabs API calls (for navbar course tabs)
export const quickTabsAPI = {
  getQuickTabs: () => api.get('/admin/quick-tabs'),
  getAvailableDegreeTypes: () => api.get('/admin/quick-tabs/available'),
  createQuickTab: (data) => api.post('/admin/quick-tabs', data),
  updateQuickTab: (id, data) => api.put(`/admin/quick-tabs/${id}`, data),
  reorderQuickTabs: (order) => api.put('/admin/quick-tabs/reorder', { order }),
  toggleQuickTab: (id) => api.put(`/admin/quick-tabs/${id}/toggle`),
  deleteQuickTab: (id) => api.delete(`/admin/quick-tabs/${id}`),
}

// Course Page Content API (for listing page blog sections)
export const coursePageContentAPI = {
  getAll: () => api.get('/admin/course-page-content'),
  getByType: (courseType) => api.get(`/admin/course-page-content/${courseType}`),
  getCourseTypes: () => api.get('/admin/course-page-content/course-types'),
  create: (data) => api.post('/admin/course-page-content', data),
  update: (courseType, data) => api.put(`/admin/course-page-content/${courseType}`, data),
  delete: (courseType) => api.delete(`/admin/course-page-content/${courseType}`),
}

// Course Types Master API (for managing standardized course types)
export const courseTypesAPI = {
  getAll: (params) => api.get('/admin/course-types', { params }),
  getById: (id) => api.get(`/admin/course-types/${id}`),
  create: (data) => api.post('/admin/course-types', data),
  update: (id, data) => api.put(`/admin/course-types/${id}`, data),
  delete: (id) => api.delete(`/admin/course-types/${id}`),
  getCategories: () => api.get('/admin/course-types/categories'),
  getAllCategories: () => api.get('/admin/course-types/categories/all'),
  createCategory: (data) => api.post('/admin/course-types/categories', data),
  updateCategory: (id, data) => api.put(`/admin/course-types/categories/${id}`, data),
  deleteCategory: (id) => api.delete(`/admin/course-types/categories/${id}`),
  reorder: (orders) => api.post('/admin/course-types/reorder', { orders }),
}

// Course Location Content API (for city/state specific pages)
export const courseLocationContentAPI = {
  getAll: (params) => api.get('/admin/course-location-content', { params }),
  getOne: (courseType, locationSlug) => api.get(`/admin/course-location-content/${courseType}/${locationSlug}`),
  getAvailableLocations: (courseType) => api.get(`/admin/course-location-content/available-locations/${courseType}`),
  create: (data) => api.post('/admin/course-location-content', data),
  update: (courseType, locationSlug, data) => api.put(`/admin/course-location-content/${courseType}/${locationSlug}`, data),
  delete: (courseType, locationSlug) => api.delete(`/admin/course-location-content/${courseType}/${locationSlug}`),
}
